...
function getLocalAcceptanceInstances()
{
  ...
  if (empty($instances) || getenv('ADT_DEV_MODE')) {
    ...
    foreach ($vhosts as $vhost) {
      ...
      $descriptor_array['DEPLOYMENT_LOG_APPSRV_URL'] = $scheme . "://" . $_SERVER['SERVER_NAME'] . ":" . $_SERVER['SERVER_PORT'] . "/logs?type=instance&file=" . $descriptor_array['DEPLOYMENT_LOG_PATH'];
      $descriptor_array['DEPLOYMENT_LOG_APACHE_URL'] = $scheme . "://" . $_SERVER['SERVER_NAME'] . ":" . $_SERVER['SERVER_PORT'] . "/logs?type=apache&file=" . getenv('ADT_DATA') . "/var/log/apache2/" . $descriptor_array['DEPLOYMENT_EXT_HOST'] . "-access.log";
      ...
      switch ($descriptor_array['PRODUCT_NAME']) {
        ...
        case 'plfenteap':
          switch ($descriptor_array['PRODUCT_BRANCH']) {
            case '7.3.x':
              $descriptor_array['PRODUCT_ADDONS_DISTRIB']="exo-agenda / exo-digital-workplace / exo-jcr / exo-jitsi / exo-ecms / exo-web-conferencing / exo-multifactor-authentication / exo-onlyoffice / exo-documents / exo-external-visio-connector / meeds-app-center / meeds-gamification / meeds-layout / meeds-wallet / meeds-kudos / meeds-perk-store / meeds-push-notifications / meeds-notes / meeds-task / meeds-analytics / meeds-poll / meeds-matrix / meeds-gamification-twitter / meeds-gamification-crowdin / meeds-pwa / meeds-ide";
              break;
            case '7.2.x':
