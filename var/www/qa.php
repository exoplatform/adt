    ...
    <tbody>
      ...
      <tr>
        <td colspan="6" class="category-row">
          <span class="group-type-tag type-qa me-2">QA</span>
          <?= "Platform " . $plf_branch . " QA environments" ?>
        </td>
      </tr>
      <?php foreach ($descriptor_arrays as $d): ?>
      <tr>
        <td class="col-center"><?= componentStatusIcon($d) ?></td>
        <td>
          <div class="d-flex align-items-center gap-2">
            <div class="min-w-0">
              <?= componentProductInfoIcon($d) ?>&nbsp;<?= componentProductOpenLink($d) ?>
              <div class="mt-1">
            ...
            <span class="ms-auto flex-shrink-0">
              <a href="<?= isset($d->DEPLOYMENT_BUILD_URL) ? $d->DEPLOYMENT_BUILD_URL . '/build?delay=0sec' : 'https://ci.exoplatform.org/view/°%20ACCEPTANCE%20°/job/platform-enterprise-' . $d->BASE_VERSION . '-' . $d->INSTANCE_ID . '-acc/build?delay=0sec' ?>"
        ...
        <td class="col-center"><?= componentDownloadIcon($d) ?>&nbsp;<?= componentProductVersion($d) ?></td>
...
<?php
$qa_auto = getGlobalQAAutoInstances();
if (isDeploymentInCategoryArray($qa_auto)):
?>
<div class="inst-table-wrap mb-4">
  <table class="inst-table">
    <thead>
    ...
    <tbody>
      <?php foreach ($qa_auto as $plf_branch => $descriptor_arrays): ?>
      <tr>
        <td colspan="6" class="category-row">
          <span class="group-type-tag type-qa me-2">Auto</span>
          <?= "Platform " . $plf_branch . " Automatic QA environments" ?>
        </td>
      </tr>
      <?php foreach ($descriptor_arrays as $d): ?>
      <tr>
