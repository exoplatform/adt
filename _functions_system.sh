  ...
  if [ "${OS}" == "windowsnt" ]; then
    ...
    if [ "${OS}" = "SunOS" ]; then
      ...
      if [ -f /etc/redhat-release ]; then
        ...
        DIST=`cat /etc/redhat-release | ${CMD_SED} s/\ release.*//`
        PSEUDONAME=`cat /etc/redhat-release | ${CMD_SED} s/.*\(// | ${CMD_SED} s/\)//`
        REV=`cat /etc/redhat-release | ${CMD_SED} s/.*release\ // | ${CMD_SED} s/\ .*//`
        ...
        PSEUDONAME=`cat /etc/mandrake-release | ${CMD_SED} s/.*\(// | ${CMD_SED} s/\)//`
        REV=`cat /etc/mandrake-release | ${CMD_SED} s/.*release\ // | ${CMD_SED} s/\ .*//`
