...
do_start_jitsi() {
  ...
  jitsi_major_version=$(echo "${DEPLOYMENT_JITSI_IMAGE_VERSION}" | grep -oP '^[0-9]+')
  if [[ ! "$jitsi_major_version" =~ ^(7|8|9|10)$ ]]; then
      echo_warn "Unsupported Jitsi major version '$jitsi_major_version', defaulting to 10"
