...
initialize_product_settings() {
  ...
  case "${ACTION}" in
    ...
    # Mandatory env vars. They need to be defined before launching the script
      ...
      configurable_env_var "DEPLOYMENT_MAILPIT_IMAGE_VERSION" "v1.29.2"
      ...
      configurable_env_var "DEPLOYMENT_CLOUDBEAVER_IMAGE_VERSION" "25.3.5-acc"
      ...
      if [[ "${PRODUCT_NAME}" =~ ^(meeds) ]]; then
        ...
        if [[ "${PRODUCT_VERSION}" =~ ^(7.2) ]]; then
              ...
              env_var "DEPLOYMENT_MYSQL_DEFAULT_VERSION" "8.4.8" # Default version of the mysql server to use
              ...
              env_var "DEPLOYMENT_POSTGRESQL_ADDON_VERSION" "2.5.2" # Default version of the jdbc postgresql driver addon to use
