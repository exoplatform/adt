...
initialize_product_settings() {
  ...
  case "${ACTION}" in
    ...
    # Mandatory env vars. They need to be defined before launching the script
      ...
      configurable_env_var "DEPLOYMENT_MAILPIT_IMAGE_VERSION" "v1.30.3"
      ...
      configurable_env_var "DEPLOYMENT_CLOUDBEAVER_IMAGE_VERSION" "26.1.1-acc"
      ...
      if [[ "${PRODUCT_NAME}" =~ ^(meeds) ]]; then
        ...
        if [[ "${PRODUCT_VERSION}" =~ ^(7.3) ]]; then
              ...
              env_var "DEPLOYMENT_ES_IMAGE_VERSION" "9.3.4"
